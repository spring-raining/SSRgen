  ...
  popupPlayWithMovieModal(image) {
    const modal = (
      <Modal className="sign-video-modal"
             onCloseButtonClick={this._onCloseButtonClick.bind(this)}>
        <MoviePlayer image={image} type={this.state.settings.pencilColor}/>
      </Modal>
    );
    this.setState({
      modal: modal,
...
class MoviePlayer extends React.Component {
  ...
  drawCharacter(time) {
    ...
    const iw = this.state.background.width;
    const ih = this.state.background.height;
      ...
      <div>
        ...
        <div>
          ...
          [<input type="text"
                 ...
                 onChange={(ev) => this.setState({characterTitle: ev.target.value})} />]
          &ensp;
          <input type="text"
                 placeholder="名前"
                 value={this.state.characterName}
                 onChange={(ev) => this.setState({characterName: ev.target.value})} />
        </div>
        <div>
          キャラ画像&ensp;
          <input type="file"
        ...
        <p><small>画像はブラウザ内のみで処理され、外部にアップロードはされません。</small></p>
      </div>
    );
  }
}
  ...
  render() {
    ...
    return (
      <div className={classNames.join(' ')}>
        <div className="character-title">[{this.props.title}]</div>
