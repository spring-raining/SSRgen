...
class Layer extends React.Component {
  ...
  saveUndoBuffer() {
    if (!undoBuffer) {
      const undoBuffer = document.createElement('canvas');
    ...
    const undoCtx = this.state.undoBuffer.getContext('2d');
  ...
  swapUndoBuffer() {
    ...
    if (!swapBuffer) {
      const swapBuffer = document.createElement('canvas');
    ...
    const undoCtx = this.state.undoBuffer.getContext('2d');
    const swapCtx = this.state.swapBuffer.getContext('2d');
